...
\textbf{Proposition. Left Derived Functor is Coeffaceable.} \ The left derived functor $L_nF$ of any right exact functor $F: \mathcal{A} \to \mathcal{B}$ is coeffacable.

\textbf{Proof.} \ Given any $A \in \mathcal{A}$ we find a projective object $P \in \mathcal{A}$ such that $P \twoheadrightarrow A$. Applying $L_iF$ to $P$ we find the projective resolution of $P$ by
\[
  \begin{tikzcd}[column sep=2.5em, row sep=2.5em]
    0 \rar & P \arrow[]{rr}{\cong} & & P \rar & 0
  \end{tikzcd}
\]
that is concentrated in degree 0 and thus has zero homology in all degress $n \geq 1$.
